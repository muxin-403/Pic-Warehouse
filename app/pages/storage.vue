...
function buildPatchBody(payload: StorageFormPayload) {
  ...
  if (payload.type === 'webdav') {
    // 用户名可为空（匿名 WebDAV）；密码留空表示编辑时保持原值
    if (payload.secrets.username || payload.secrets.password) {
      body.secrets = {
        username: payload.secrets.username,
        password: payload.secrets.password
      }
    }
  } else if (payload.secrets.accessKeyId || payload.secrets.secretAccessKey) {
    body.secrets = {
      accessKeyId: payload.secrets.accessKeyId,
      secretAccessKey: payload.secrets.secretAccessKey
    }
  }
  ...
  if (formMode.value === 'create') {
    if (payload.type === 'webdav') {
      if (!payload.config.baseUrl) {
        toast.add({ title: t('storage.webdavUrlRequired'), color: 'warning' })
        return
      }
    } else if (!payload.secrets.accessKeyId || !payload.secrets.secretAccessKey) {
