...
1. 以管理员登录，打开 **存储** → **添加存储**。
2. 选择类型：**本地磁盘**、**S3 兼容**（含 R2 / COS / OSS 预设）或 **WebDAV**。
3. 填写连接信息（桶名、Endpoint、Access Key；或服务器地址、用户名、密码等）。
4. 保存后可 **设默认**，新上传将写入该后端。

新建对象存储后端的 **存储标识** 按类型自动生成前缀：本地为 `local`；R2 为 `r2-*`；COS 为 `cos-*`；OSS 为 `oss-*`；AWS S3 及自定义 S3 兼容为 `s3-*`；WebDAV 为 `webdav-*`（创建后不可修改，图片索引会引用该 ID）。
...
实现基于标准 WebDAV 方法（`PUT` / `GET` / `HEAD` / `DELETE` / `MKCOL` / `PROPFIND`），不依赖第三方客户端库，已在 Nextcloud、坚果云、Alist、`wsgidav` 等实现上验证；「测试连接」会依次校验鉴权、读取与写入权限。
...
**UI 中配置的后端信息存于 SQLite**，与环境变量并存时以管理界面为准管理多后端。
