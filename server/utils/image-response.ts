...
import { buildWebdavObjectPath, parseWebdavConfig } from './storage/webdav'
...
export function buildPublicImageUrl(
  ...
  if (backendRow.type === 'webdav') {
    const config = parseWebdavConfig(backendRow.config_json)
    const objectPath = buildWebdavObjectPath(config?.path, key)
    const publicPath = hideFolder ? toPublicImagePath(key, true) : objectPath
    return `${base}/${publicPath}`
  }
  return `${base}/${path}`
}
...
export function resolveImageUrl(
  ...
  if (envConfig && (!backendId || backendId === `${envConfig.type}-primary`)) {
    if (envConfig.servingMode === 'public' && envConfig.publicUrl) {
      const base = envConfig.publicUrl.replace(/\/$/, '')
      const objectPath = envConfig.type === 'webdav'
        ? buildWebdavObjectPath(envConfig.config.path as string | undefined, key)
        : buildObjectKey(envConfig.config.prefix as string | undefined, key)
      const publicPath = hideFolder ? toPublicImagePath(key, true) : objectPath
      return `${base}/${publicPath}`
    }
